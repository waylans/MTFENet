...
class BasePredictor:
    ...
    def save_preds(self, vid_cap, idx, save_path):
        ...
        if self.dataset.mode == 'image':
            ...
            # 调整 mask1 和 mask2 的尺寸与原始图像一致
            mask1_resized = cv2.resize(mask1, (im0.shape[1], im0.shape[0]))
            mask2_resized = cv2.resize(mask2, (im0.shape[1], im0.shape[0]))

            color_mask1 = np.stack([mask1_resized * 0, mask1_resized * 255, mask1_resized * 0], axis=-1)
            color_mask2 = np.stack([mask2_resized * 255, mask2_resized * 0, mask2_resized * 0], axis=-1)

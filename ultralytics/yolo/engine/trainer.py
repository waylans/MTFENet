    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                self.scaler.scale(self.loss).backward(retain_graph=False)  ###### retain_graph=False Free the GPU memory,
                ...
                if ni - last_opt_step >= self.accumulate:
                    self.optimizer_step()
                    last_opt_step = ni
                self.loss = self.loss.detach()  ###### Free the GPU memory

...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment, Detect_DE)):
...
#         return x
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            if isinstance(m, (Detect, Segment,Detect_DE)):  # if it's a task head
                outputs.append(x)
            # y.append(x)
            y.append(x if m.i in self.save else None)  # save output
            if visualize:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        for m in self.model:
            if isinstance(m, (Detect, Segment, Detect_DE)):
                s = 256  # 2x min stride
                # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 增加的，将模型放入GPU运行
...
class MultiModel(MultiBaseModel):
    ...
    def __init__(self, cfg='yolov8-bdd.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        for m in self.model:  #
        # m = self.model[-1]  # Detect()
            if isinstance(m, (Detect, Segment, Pose,Detect_DE)):
                s = 256  # 2x min stride
                # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 增加的，将模型放入GPU运行
                m.inplace = self.inplace
                # m.cuda()  # 增加的，将模型放入GPU运行
                ...
                ######  do not move, otherwise the gradient will not backward to segmentation head. Only detection head will implement
                try:
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Segment,Detect_DE):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3 ,FTM):
        ...
        elif m in (Detect, Segment, Pose, Detect_DE):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Detect, Detect_DE):
                return 'detect'
